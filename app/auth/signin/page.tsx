  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://nextbackend-d5ze.onrender.com/auth/signin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include", // Ensure authentication cookies work
        body: JSON.stringify({ email, password }),
      });
    ...
    <div className="flex">
      ...
      <div className="flex-1 flex h-screen items-center justify-center px-4">
        <form className="w-[90%]" onSubmit={handleSubmit}>
          ...
          <div className="mt-6">
            <input type="email" placeholder="Email..." className="w-full h-14 rounded-lg outline-none px-4 bg-[#312F2F] text-white" value={email} onChange={(e) => setEmail(e.target.value)} />
            {errors.email && <p className="text-red-500 text-sm">{errors.email}</p>}
          </div>

          <div className="mt-6">
            <input type="password" placeholder="Enter Password..." className="w-full h-14 rounded-lg outline-none px-4 bg-[#312F2F] text-white" value={password} onChange={(e) => setPassword(e.target.value)} />
            {errors.password && <p className="text-red-500 text-sm">{errors.password}</p>}
          </div>
          ...
          <div className="mt-6">
            <button type="submit" className="w-full block bg-[#00FF38] text-black text-lg font-semibold capitalize h-14 px-4 rounded-lg" disabled={loading}>
              {loading ? "Logging in..." : "Login"}
            </button>
          </div>
